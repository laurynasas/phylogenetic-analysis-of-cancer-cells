...
# from k_medoid_clustering import read_simulated_data_file
import helpers
        ...
        for i in range(self.number_of_iterations):
            ...
            for k in xrange(self.k_clusters):
                ...
                for i in xrange(self.d_pixels):
                    miu_pixel_distributions[k, i] *= (np.random.random_sample() * 0.5) + 0.25
                    normalize += miu_pixel_distributions[k, i]

                for i in xrange(self.d_pixels):
                    miu_pixel_distributions[k, i] /= normalize

            z_matrix = np.zeros((self.n_images, self.k_clusters), dtype=np.float64)

            z_matrix[:, 0] = np.ones(self.n_images)
            ...
            pi_proportion_of_images = np.sum(z_matrix, axis=0) * 1.0 / self.n_images
...
if __name__ == '__main__':
    ...
    unique_rows, full_data_dict, full_info = helper.read_simulated_data_file(dir)

    bmm = BMM(k_clusters,unique_rows,full_data_dict,full_info,number_of_iterations=20)

    for j in range(1,loop_times+1):
        print bmm.do_clustering()
